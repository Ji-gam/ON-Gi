...
class AuthService:
    ...
    async def _assert_unique(self, email: str | None, nickname: str, phone_number: str | None) -> None:
    ...
    async def _is_email_verified(self, email: str) -> bool:
        return (
    ...
    async def _is_phone_verified(self, phone_number: str) -> bool:
        phone_hash = security.hash_phone(validators.normalize_phone_number(phone_number))
        return (
            await self.session.scalar(
    ...
    async def signup(self, data: SignUpRequest) -> AuthResult:
        ...
        if config.require_email_verification() and not await self._is_email_verified(email):
            raise HTTPException(status.HTTP_400_BAD_REQUEST, "이메일 인증이 완료되지 않았습니다.")
        if config.require_phone_verification() and not await self._is_phone_verified(data.phone_number):
            raise HTTPException(status.HTTP_400_BAD_REQUEST, "휴대폰 본인확인이 완료되지 않았습니다.")

        await self._assert_unique(email, data.nickname, data.phone_number)
